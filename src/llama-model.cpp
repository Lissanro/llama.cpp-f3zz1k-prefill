...
const char * llm_type_name(llm_type type) {
    switch (type) {
        ...
        case LLM_TYPE_432B_A28B:     return "432B.A28B";
...
llama_memory_i * llama_model::create_memory(const llama_memory_params & params, const llama_cparams & cparams) const {
    ...
    switch (arch) {
        ...
        default:
            {
                // The MTP head is dense-attention only on hybrid Qwen3-Next/3.5/3.6, GigaChat 3.5
                // and Bailing-MoE 3, so use a plain attention KV cache for the MTP context instead of
                // the hybrid wrapper.
                const bool mtp_on_hybrid_qwen =
                    params.ctx_type == LLAMA_CONTEXT_TYPE_MTP &&
                    (arch == LLM_ARCH_QWEN3NEXT || arch == LLM_ARCH_QWEN35 || arch == LLM_ARCH_QWEN35MOE ||
                     arch == LLM_ARCH_GIGACHAT35 || arch == LLM_ARCH_BAILINGMOE3);
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    } else if (arch == LLM_ARCH_QWEN3NEXT || arch == LLM_ARCH_QWEN35 || arch == LLM_ARCH_QWEN35MOE || arch == LLM_ARCH_GIGACHAT35 || arch == LLM_ARCH_QWEN4EXP || arch == LLM_ARCH_MINIMAX_01) {
                        filter_attn = [&](uint32_t il) {
